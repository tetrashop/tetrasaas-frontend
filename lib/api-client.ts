...
class APIClient {
  ...
  async getNLPosts(page: number = 1, limit: number = 25): Promise<NLPostsResponse> {
    try {
  ...
  private getMockNLPosts(page: number, limit: number): NLPostsResponse {
    ...
    for (let i = 1; i <= limit; i++) {
      ...
      mockPosts.push({
        ...
        category: ['تحلیل احساسات', 'طبقه‌بندی متن', 'تشخیص موجودیت'][postId % 3],
        author: `محقق ${postId}`,
  ...
  private getMockServices(): Service[] {
    const categories = ['پردازش تصویر', 'پردازش متن', 'تبدیل صوت', 'تحلیل داده', 'یادگیری ماشین'];
    
    return Array.from({ length: 26 }, (_, i) => ({
      id: i + 1,
      name: `سرویس ${String.fromCharCode(65 + i)}: ${[
        'تشخیص چهره هوشمند', 'تبدیل متن به گفتار', 'ترجمه عصبی', 'خلاصه‌ساز متون',
        'تولید کد خودکار', 'تحلیل احساسات', 'تشخیص اشیا', 'پاسخگوی هوشمند',
        'تولید تصویر هنری', 'تشخیص کلاهبرداری', 'پیش‌بینی بازار', 'تشخیص پزشکی',
        'بهینه‌سازی کد', 'تولید محتوا', 'دستیار صوتی', 'تحلیل ویدیو',
        'تشخیص پلاک', 'تجزیه احساسات صوتی', 'تشخیص دستخط', 'تولید موسیقی',
        'تشخیص اسپم', 'پیشنهاد هوشمند', 'تشخیص زبان بدن', 'تولید زیرنویس',
        'تحلیل شبکه‌های اجتماعی', 'دستیار برنامه‌نویسی'
      ][i]}`,
...
const apiClient = new APIClient(
  process.env.NEXT_PUBLIC_API_BASE_URL || 'https://tetrashop-projects.vercel.app/api'
);
